...
for study = {'bmrk5', 'paingen', 'spacetop'}
    for hemi = {'lh' 'rh'}
        pdata = fmri_data(which(sprintf('%s_%s_dk_%s.nii.gz', hemi{1}, study{1}, space_description)));
...
%% save nifti version
pdata.fullpath = sprintf('desikan_killiany_%s.nii', space_description);
pdata.write('overwrite')
gzip(pdata.fullpath)
delete(pdata.fullpath);
writetable(table(atlas_obj.labels', atlas_obj.label_descriptions, 'VariableNames', {'labels', 'label_descriptions'}), 'desikan_killiany_labels.csv');