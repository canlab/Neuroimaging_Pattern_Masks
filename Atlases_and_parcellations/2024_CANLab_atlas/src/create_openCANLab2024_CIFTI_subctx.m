function create_CANLab2024_CIFTI_subctx(SPACE,SCALE,res,atlas_obj)
    ...
    this_dir = dir(which('create_openCANLab2024_CIFTI_subctx.m'));
    %% output label files
    % we need these to match Glasser parcel labels to canlab atlas labels
    labels = atlas_obj.labels;
    fid = fopen(sprintf('%s/openCANLab2024_%s_%s_%dmm_cifti_canlab_labels.txt', this_dir.folder, SPACE, SCALE, round(res)),'w+');
    ...
    fid = fopen(sprintf('%s/openCANLab2024_%s_%s_%dmm_cifti_glasser_labels.txt', this_dir.folder, SPACE, SCALE, round(res)),'w+');
    for i = 1:num_regions(atlas_obj)
        fprintf(fid, '%d %s\n',i,labels{i});
    end
    fclose(fid);
    ...
    if any(atlas_obj.volInfo.mat ~= cifti_mask.volInfo.mat,'all')
        ...
        if n_labels ~= num_regoins(atlas_obj)
            ...
            delete(sprintf('%s/openCANLab2024_%s_%s_%dmm_cifti_index_labels.txt', this_dir.folder, SPACE, SCALE, round(res)));
        end
    end
    ...
    fid = fopen(sprintf('%s/openCANLab2024_%s_%s_%dmm_cifti_vols.txt', this_dir.folder, SPACE, SCALE, round(res)),'w+');
    for i = 1:n_roi
