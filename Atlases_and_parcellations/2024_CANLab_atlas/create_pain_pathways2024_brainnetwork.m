% Create the painpathways atlas object
% ----------------------------------------------------------------------
...
thal2 = select_atlas_subset(canlab2018, {'Thal'});
...
vplm = select_atlas_subset(thal, [{'VPL' 'VPM'}]);

% intralaminar thalamus is comprised of intrlaminar and midline groups
% ilthal = select_atlas_subset(thal, [{'Intralam' 'Midline'}], 'labels_2', 'flatten');

ilthal = select_atlas_subset(thal, [{'Intralam'}], 'labels_2', 'flatten');
% midline = select_atlas_subset(thal, {'Midline'}, 'labels_2', 'flatten');

% midline = select_atlas_subset(thal2, {'Midline'}, 'labels', 'flatten');

% mediodorsal nucleus is a large 'association' nucleus in the thalamus,
% connecting to multiple limbic areas (e.g., amygdala)

md = select_atlas_subset(thal, {'MD'}, 'flatten');
md.labels='Thal_MD';
...
hy = select_atlas_subset(atlas_obj, {'hypothalamus'}, 'labels_2', 'flatten');


%% Add brainstem regions

% pbn = select_atlas_subset(atlas_obj, {'pbn'}, 'labels_3');
% pbn = select_atlas_subset(atlas_obj, {'Parabrachial'}, 'label_descriptions', 'flatten');
% pbn = select_atlas_subset(atlas_obj, {'Parabrachial'}, 'label_descriptions');
% pbn = select_atlas_subset(atlas_obj, {'Parabrachial nuclei'}, 'labels_2', 'flatten');

pbn = merge_atlases(select_atlas_subset(canlab2024, {'Parabrachial nuclei_L'}, 'labels_2', 'flatten'), ...
    select_atlas_subset(canlab2024, {'Parabrachial nuclei_R'}, 'labels_2', 'flatten'));

pag = select_atlas_subset(atlas_obj, {'PAG'}, 'flatten');
% rvm = select_atlas_subset(atlas_obj, {'rvm'});

rvm = select_atlas_subset(canlab2018, {'rvm'});

med=select_atlas_subset(atlas_obj, {'Medulla'}, 'labels_3');
...
amy_finegrained = select_atlas_subset(atlas_obj, {'Amygdala'}, 'labels_3');
amy_finegrained = split_atlas_into_contiguous_regions(amy_finegrained);

amy = select_atlas_subset(atlas_obj, {'Amygdala'}, 'labels_3', 'flatten');
...
% clean up - remove small 3rd region in L angular gyrus
dpins = select_atlas_subset(dpins, 2:3);
...
s2 = split_atlas_by_hemisphere(s2);

s2.labels = {'S2_R' 'S2_L'};
s2.labels_2= {'OP4_SII+_R' 'OP4_SII+_L'};
s2.labels_3 = {'somatomotor_operculum_R' 'somatomotor_operculum_L'};
s2.labels_4 = {'Glasser2016 (Petre2023 volumetric projection)' 'Glasser2016 (Petre2023 volumetric projection)'};
...
o2 = montage(pain_pathways_finegrained, 'trans');
