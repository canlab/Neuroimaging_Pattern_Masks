...
%% Alternate: Cutaways, VMPFC -> NAC


pp = addbrain('cutaway', 'ycut_mm', -30);

set(pp(1), 'Visible', 'off');
set(pp([2 3 8:12]), 'FaceAlpha', .1);
set(pp([4 7]), 'FaceAlpha', .8);
set(pp([7]), 'FaceAlpha', .7);

%vmpfc = canlab_load_ROI('vmpfc');
[hpatch, cl] = cluster_image_sphere([0 40 -15], 'color', [.2 .8 .2], 'radius', 10);
vmpfc = cluster2region(cl);

nac = canlab_load_ROI('nac');


color = [.15 .8 .3];         % cannot exceed .9 for any value, because of random addition below.
thickness = .1;
bendpercent = .3;
nstreamlines = 30;
...
%         if sigmat(rindx, rindx2)
            ...
            for i = 1:nstreamlines % number of streamlines
                randval = unifrnd(-.15, .15);
                out = nmdsfig_tools('connect3d',vmpfc(1).mm_center + 10*randval, nac(2).mm_center + 10*randval, color + randval, thickness, bendpercent+randval);
            end
...
nac_han = isosurface(nac);
set(nac_han, 'FaceColor', [.2 .8 .2]);
...
saveas(gcf, 'vmpfc_nac_pathway_surface_rendering1.png');
