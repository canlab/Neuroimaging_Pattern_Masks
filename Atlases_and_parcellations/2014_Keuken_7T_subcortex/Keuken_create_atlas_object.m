...
atlas_name = 'Keuken_7T';
space_description = 'MNI152 space';
references = 'Keuken 2014 Neuroimage';
...
parcellation_file = which('Nonlinear_combined_masks_threshold033.nii.gz');

imgs = filenames('/Users/tor/Google_Drive/CanlabDataRepository/ROI_masks_and_parcellations/Downloaded_image_collections/Subcortex_Keuken2014/Non-Linear/normalized/Non*zed.nii', 'absolute');

% remove STR for overall striatum
wh = ~cellfun(@isempty, strfind(imgs, 'STR'));
imgs(wh) = [];
...
labels = {'L_RN' 'R_RN' 'L_SN' 'R_SN' 'L_STN' 'R_STN' 'L_GPe' 'R_GPe' 'L_GPi' 'R_GPi'};

% Create object
% -----------------------------------------------------------------------

atlas_obj = atlas(imgs, ...
...
if doresample
    % Resample to 1 mm, save space
    
    reference_file = which('HCP-MMP1_on_MNI152_ICBM2009a_nlin.nii');

    atlas_obj = resample_space(atlas_obj, fmri_data(reference_file));
