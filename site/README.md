...
Branding uses the unmodified `ncs_thumbnail.png` supplied by Tor Wager, served as `brand/neuromarkers-ncs.png` for the hero, logo, favicon, and social previews. A 2D canvas mask reveals cursor-local color for 3.6 seconds before fading to grayscale. Occasional soft blooms appear while the hero is visible; reduced-motion preferences disable spontaneous blooms. Touch and keyboard interactions also reveal local color. Previous NPS renderings and their offline preparation script are retained as backups. Map-specific catalog tile previews remain tied to their respective maps.
