...
Branding uses CANlab’s canonical left-cutaway mesh with the NPS weights from the public CANlab paradigm repository. `scripts/prepare_brand.py` renders committed static assets offline (matplotlib required only for regeneration). The hero reveals the pattern on hover, keyboard focus, or tap, with no extra WebGL context. The warm-paper light theme matches Science of Placebo and the CANlab homepage. The introduction cites Woo et al. (2017); root and study social cards share the Neuromarkers brand graphic.
