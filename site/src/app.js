...
function renderFilters(){
 ...
 return `<fieldset><legend>${label}</legend>${values.map(t=>`<label class="filter-option"><input type="checkbox" data-axis="${axis}" value="${esc(t)}" ${filters[axis].includes(t)?'checked':''}><span>${esc(t)}</span><small>${catalog.studies.filter(s=>s[axis].includes(t)||(axis==='targets'&&s.maps.some(m=>m.targets.includes(t)))).length}</small></label>`).join('')}</fieldset>`}).join('');
}
...
function applyThreshold(){
 ...
 nv.setOpacity(nv.volumes.length-1,display.opacity);nv.setOpacity(0,display.anatomy);nv.updateGLVolume();
 for(const side of ['positive','negative']){const c=side==='positive'?positive:negative;$(side+'-cutoff').textContent=map.stats[side].count?`${map.stats[side].count.toLocaleString()} ${side} voxels · ${Number.isFinite(c)?'magnitude ≥ '+c.toPrecision(4):'hidden'}`:'No values of this sign';}
...
async function loadMap(selected){
 ...
 try{
  if(!modules){const css=document.createElement('link');css.rel='stylesheet';css.href=url(catalog.assets.css);document.head.prepend(css);modules=await Promise.all([import(url(catalog.assets.viewer)),import(url(catalog.assets.niivue))]);atlasLabels=await fetchJSON(catalog.assets.shared+'atlas-labels.json');}
  ...
  const selects=$('canlab-niivue-controls').querySelectorAll('select');if(selects.length>1&&['outline','shaded','off'].includes(p.get('atlas'))){selects[selects.length-1].value=p.get('atlas');selects[selects.length-1].dispatchEvent(new Event('change'));}
  ...
  try{
   ...
   surfaceOriginal=surface.meshes.map(m=>m.layers[0].values.slice());surface.setRenderAzimuthElevation(p.has('azimuth')?Number(p.get('azimuth')):110,p.has('elevation')?Number(p.get('elevation')):15);$('hemisphere').value=['L','R'].includes(p.get('hemisphere'))?p.get('hemisphere'):'both';hemispheres();applyThreshold();$('surface-status').textContent='';
...
async function main(){
 ...
 $('threshold-mode').onchange=()=>{const next=$('threshold-mode').value;if(next==='absolute'){display.positive=cutoff(map.stats.positive,display.positive,display.mode);display.negative=cutoff(map.stats.negative,display.negative,display.mode);for(const side of ['positive','negative'])if(!Number.isFinite(display[side]))display[side]=map.stats[side].max;display.sync=false;}else{display.positive=display.negative=20}display.mode=next;syncControls();applyThreshold();remember()};
 ...
 $('png').onclick=()=>{try{if(!nv)throw Error('Interactive viewer unavailable.');nv.drawScene();surface?.drawScene();const a=$('gl'),b=$('surface-gl'),out=document.createElement('canvas');out.width=a.width+(surface?b.width:0);out.height=Math.max(a.height,surface?b.height:0)+80;const c=out.getContext('2d');c.fillStyle='#080d12';c.fillRect(0,0,out.width,out.height);c.drawImage(a,0,0);if(surface)c.drawImage(b,a.width,0);c.fillStyle='white';c.font='20px sans-serif';c.fillText(map.name,20,out.height-45);c.font='16px sans-serif';c.fillText(`${display.mode}: positive ${Number(display.positive).toPrecision(4)}, negative ${Number(display.negative).toPrecision(4)} · ${display.sign}`,20,out.height-18);out.toBlob(blob=>{if(blob)downloadBlob(blob,map.id+'.png')})}catch(e){$('action-status').textContent=e.message}};
